...
const MonthSocMinMax = ({ dataSet }) => {
  const [showInfo, setShowInfo] = useState(false);

  // Take the SOC min values coordinates
  const SOCminValues = dataSet["Months Values"]
    .map((item, index) => {
      if (index >= 1) {
        return {
          x: index,
          y: item["SOC avg min"] * 100,
        };
      } else {
        return null;
      }
  ...
  const SOCmaxValues = dataSet["Months Values"]
    .map((item, index) => {
      if (index >= 1) {
        return {
          x: index,
          y: item["SOC avg max"] * 100,
        };
      } else {
        return null;
      }
  ...
  return (
    <View>
      <View style={styles.chartHeader}>
        <Text style={styles.chartName}>SOC min/max</Text>
