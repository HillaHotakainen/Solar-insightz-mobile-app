...
const MonthBatTempMinMax = ({ dataSet }) => {
  const [showInfo, setShowInfo] = useState(false);

  // Take the Bat temp min values coordinates
  const BatTempMinValues = dataSet["Months Values"]
    .map((item, index) => {
      if (index >= 1) {
        return {
          x: index,
  ...
  const BatTempMaxValues = dataSet["Months Values"]
    .map((item, index) => {
      if (index >= 1) {
        return {
          x: index,
...
export default MonthBatTempMinMax;
